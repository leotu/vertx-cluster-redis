...
class ConnectionHolder {
  ...
  synchronized void writeMessage(ClusteredMessage message) {
    if (connected) {
      ...
      pending.add(message);
      //log.debug("*** writeMessage for server " + serverID + ", pending.size: " + pending.size());
    }
  }

  void close() {
	log.debug("*** close for server " + serverID);
  ...
  private void close(Throwable cause) {
    ...
    synchronized (this) {
      ...
      if (pending != null) {
    	log.debug("*** close for server " + serverID + ", pending.size: " + pending.size());
...
//        	ClusteredMessage cmsg = (ClusteredMessage)msg;
    	  ...
    	  log.debug("*** close for server " + serverID + ", pending is null");
      }
    }
    try {
      client.close();
